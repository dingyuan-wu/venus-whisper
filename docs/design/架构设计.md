...
| `list_personas` | — | 内置在前、本地在后；同名的内置/本地互相标注：内置 `synced`/`identical`，本地 `origin`/`modified`（按 prompt 与 character_bible 文本比较） |
| `get_persona` / `set_persona` | `id?, file?` | 读写指定人格的 prompt，`file = "bible"` 读 character_bible.md；内置与角色目录只读 |
| `sync_persona` | `id, overwrite` | 内置人格复制到本地，返回本地 id |
| `get_user_avatar` | — | `~/.venus-whisper/user.*` 的 data URL，无则 null |
| `list_skills` / `get_skill` / `set_skill` / `sync_skill` | — | 与人格同构；`set_skill` 只写本地 |
| `estimate_context` | `session_id` | 系统提示词 + 历史的 token 估算，右侧栏在没有新调用时据此显示占用 |
...
| `agent:usage` | `{ call, prompt_tokens, completion_tokens, system_tokens, estimated, context_window, breakdown }` 每次 LLM 调用一条；`breakdown` 把输入拆成人格 / 技能 / 输出约定 / 工具定义 / 历史对话 / 本次消息 / 本轮工具往返（均为估算）；请求带 `stream_options.include_usage`，服务端不给 usage 时用 `tokens.rs` 的本地估算（CJK 按字、其余 4 字符 1 token）并置 `estimated` |
...
- `personas/<name>.md` 与 `personas/<slug>/`：本地人格，可编辑。内置人格是 `src-tauri/personas/` 用 `include_dir` 编译进二进制的只读副本，id 为 `builtin/<name>`，默认 `builtin/venus`，不落盘；`sync_persona` 命令把内置项复制到本地目录（已存在需 `overwrite`）。旧版根目录的 `persona.md` 首次启动迁移为 `personas/venus.md`。角色目录是 `skills/role-replication-skill-v2.md` 的产物，只读取 `prompt.md`，`role.json` 的 `name`/`description` 用于展示，`avatar.*`（md 人格为同名图片）由 Rust 读成 data URL 返回，聊天中 assistant 的头像与名字跟随当前人格；`[agent] persona` 选中其一，找不到时回落默认
- `skills/*.md`：本地 skill。内置 skill 是 `src-tauri/skills/` 用 `include_dir` 嵌入的只读副本（id `builtin/<name>`），列表、同步、对比逻辑与人格一致；`[agent] skills` 中启用的 skill 正文在组装 system prompt 时追加到 persona 之后
- `[ui] debug`：调试模式，前端据此显示 `agent:usage`；`[[llm.models]].context_window` 为上下文窗口，0 表示未知
- `[llm]`：`active` + `[[llm.models]]` 多模型，`active_model()` 取当前项；旧版扁平字段读入时自动包成 id `default` 的条目
- `[user] name` 与 `user.<ext>`：用户显示名与头像，头像读法与人格头像相同（`get_user_avatar` 返回 data URL）
